...
void RysujInterfejsWiezyczki() {
    ...
    ImGui::Begin("Stacja Nadawcza - Wiezyczka STM32", nullptr, ImGuiWindowFlags_NoCollapse);

    ImGui::Text("Status Portu:"); ImGui::SameLine();
    if (hSerial != INVALID_HANDLE_VALUE) {
        ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "POLACZONO (COM3)");
    }
    else {
    ...
    ImGui::RadioButton("Automatyczny / Test", &trybPracy, 1);
    ImGui::Separator();

    ImGui::Text("Sterowanie:");

    if (ImGui::Button("W - Gora", ImVec2(100, 40)) || ImGui::IsKeyPressed(ImGuiKey_W, true)) {
        WyslijKomende('w', "Gora");
    }

    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 110);
    if (ImGui::Button("S - Dol", ImVec2(100, 40)) || ImGui::IsKeyPressed(ImGuiKey_S, true)) {
        WyslijKomende('s', "Dol");
    }

    if (ImGui::Button("A - Lewo", ImVec2(100, 40)) || ImGui::IsKeyPressed(ImGuiKey_A, true)) {
        WyslijKomende('a', "Lewo");
    }
    ImGui::SameLine();

    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 102);
    if (ImGui::Button("D - Prawo", ImVec2(100, 40)) || ImGui::IsKeyPressed(ImGuiKey_D, true)) {
    ...
    if (ImGui::Button("ZATRZYMANIE AWARYJNE (SPACJA)", ImVec2(-1, 50)) || ImGui::IsKeyPressed(ImGuiKey_Space, true)) {
    ...
    ImGui::Text("Ostatnio wyslano: "); ImGui::SameLine();
